...
public class ComputerView extends View {
  ...
  public ComputerView(Context context, AttributeSet attrs) {
    super(context, attrs);
    ...
    for(int row=0;row<11; row++){
      for(int col=0;col<11; col++) {
        ...
        Game.computer_board[row][col] = new BoardCell(col_width, col_width,
  ...
  for(int row=0; row < 11;row++){
      for(int col=0; col < 11;col++){
        if(Game.computer_board[col][row].hasShip())
          computer_drawCell("S", row,col,canvas);
        if(Game.computer_board[col][row].isWaiting())
          computer_drawCell("W", row, col,canvas);
        if(Game.computer_board[col][row].hasMiss())
          computer_drawCell("M", row, col,canvas);
        if(Game.computer_board[col][row].hasHit())
          computer_drawCell("*", row, col,canvas);
  ...
  void computer_drawCell( String contents, int row, int col, Canvas canvas ){
    canvas.drawText( contents, Game.computer_board[col][row].getBottomLeft().x,
        Game.computer_board[col][row].getBottomLeft().y , paint);
  }
